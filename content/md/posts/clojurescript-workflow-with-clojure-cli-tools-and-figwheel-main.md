...
[clj-new](https://github.com/seancorfield/clj-new) uses the Leiningen template format and some template developers will include project configuration for Clojure CLI. Each template designer is responsiblity as to which project and build tools it supports.
...
In a terminal, use the following comand:
...
> The `:project/new` alias is defined in [practicalli/clojure-deps-edn user-level configuration] and supports `-M`, `-X` and `T` execution flags.
...
* `:build` runs figwheel-main which generates JavaScript from the the ClojureScript code in the project
...
JavaScript interop code also works from the Rebel prompt, e.g. `(js/alert "Notification from the command line")` will display an altert in the browser.
...
> Running tests this way may not be as fast as using the continuous testing appoach (covered next)
...
![Figwheel-main - tests - auto-testng host page](https://github.com/practicalli/graphic-design/blob/live/clojurescript/figwheel/figwheel-main-tests-auto-testing-host-page.png)
...
Using Figwheel provides an simple way to develop, test and deploy ClojureScript projects, providing instant feeback as you develop to see exactly what the code does and help minimise bugs and avoid inapproprate design choices.
